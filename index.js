 ...
 */
function typewrite() {
    let str = "<我愿意为你，写满整个世界的情诗<因为我喜欢你，因为......我也想给你哪怕一点点的浪漫<因为当代码不止是代码的时候，它就有了新的意义。<"; //利用<作为换行符 -作为空格
    let strp = "";
    let i = 0;
...
let s = 'a';
let records = 0; //防止出现多个打字function byaei
...
function fl(e) {
    if (clickb == true) {
        ...
        if ((key <= 'z' && key >= 'a') || (key <= 'Z' && key >= 'A')) //避免大小写的错误
        {
            ...
            if (s.indexOf("YINGWXHN") != -1 && records == 0) {
                setTimeout(function() {
                    records = 1;
                    bac.style.opacity = 0;
                    box.style.opacity = 0;
